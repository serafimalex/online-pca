...
@njit(parallel=True)
def get_new_vals_numba(iq, x, d):
    max_val = NEG_INF32
    max_col = 0
    for s in prange(iq + 1, d):
        score = compute_score_cf_numba(iq, s, x, d)[2]
        if score > max_val:
            max_val = score
            max_col = s
...
@njit(parallel=True)
def get_new_vals_numba2(jq, x, d, n):
    max_val = NEG_INF32
    max_col = 0
    for s in prange(jq + 1, n):
        score = compute_score_cf_numba(jq, s, x, d)[2]
        if score > max_val:
            max_val = score
            max_col = s
    return max_val, max_col
...
def get_new_vals_col_numba(iq, x, d):
    ...
    max_row = 0
    for r in range(iq):
        score = compute_score_cf_numba(r, iq, x, d)[2]
        if score > max_val:
            max_val = score
            max_row = r
...
def get_new_vals_col_numba2(jq, x, d, p):
    ...
    for r in range(min_val):
        score = compute_score_cf_numba(r, jq, x, d)[2]
        if score > max_val:
            max_val = score
            max_row = r
    return max_val, max_row
    ...
    def fit(self, trueX, u = None):
        ...
        for q in range(self.n_iter):
            ...
            if self.use_heap == "optimized_heap":
                ...
                max_val, max_col = get_new_vals_numba(iq, x, d)
                self.heap.update_row_top1(iq, max_val, max_col)

                if jq < self.p:    
                    max_val, max_col = get_new_vals_numba2(jq, x, d, n)
                    self.heap.update_row_top1(jq, max_val, max_col)

                max_val, max_row = get_new_vals_col_numba(iq, x, d)
                self.heap.update_col_top1(max_val, max_row, iq)

                max_val, max_row = get_new_vals_col_numba2(jq, x, d, self.p)
                self.heap.update_col_top1(max_val, max_row, jq)
